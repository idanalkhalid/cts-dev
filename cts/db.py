...
class SpiderFootDb:
    ...
    createSchemaQueries = [
        ...
        "CREATE TABLE tbl_custom_profiles ( \
        ...
        "CREATE TABLE tbl_users ( \
            ...
            api_key      VARCHAR NOT NULL UNIQUE DEFAULT (lower(hex(randomblob(10)))), \
        ...
        "CREATE TABLE tbl_scan_instance ( \
            ...
            created_by       VARCHAR, \
            monitor_id       INTEGER REFERENCES tbl_monitors(id), \
            tab              VARCHAR, \
            scan_type        VARCHAR DEFAULT 'basic', \
        ...
        "CREATE TABLE tbl_scan_score ( \
            scan_instance_id    INTEGER NOT NULL REFERENCES tbl_scan_instance(guid), \
        ...
        "CREATE TABLE tbl_scannodes ( \
            id             INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, \
            scan_id        VARCHAR NOT NULL REFERENCES tbl_scan_instance(guid), \
            parent_id      INTEGER, \
            data           VARCHAR, \
            linked_scan    VARCHAR REFERENCES tbl_scan_instance(guid) \
        )",
        "CREATE TABLE tbl_monitors ( \
            ...
            investigation     INTEGER REFERENCES tbl_investigations(id), \
            usecase           VARCHAR, \
            types             VARCHAR, \
            profile           VARCHAR, \
            module_categories VARCHAR, \
            tab               VARCHAR, \
            target_type       VARCHAR, \
            last_run          INTEGER DEFAULT 0, \
            use_stream        INT NOT NULL DEFAULT 0 \
        )",
        "CREATE TABLE tbl_scan_investigation_relationships ( \
            scan_guid        VARCHAR NOT NULL REFERENCES tbl_scan_instance(guid), \
            investigation_id INTEGER NOT NULL REFERENCES tbl_investigations(id) ON DELETE CASCADE \
        )",
    ...
    categories = [
        ['IP_REPUTATION', 'IP Reputation : Number of IP form domain/sub domain that has bad reputation because of list as malicious activity',
            'MALICIOUS_IPADDR,MALICIOUS_AFFILIATE_IPADDR,MALICIOUS_SUBNET,MALICIOUS_NETBLOCK,MALICIOUS_ASN,MALICIOUS_SUBNET,MALICIOUS_NETBLOCK', 'IP REPUTATION'],
        ['APPLICATION_VULNERABILITY', 'Application Vulnerability: Number of vulnerability that listed in the applications used in this domain or ip address',
            'VULNERABILITY,SSL_CERTIFICATE_MISMATCH,SSL_CERTIFICATE_EXPIRED,SSL_CERTIFICATE_EXPIRING', 'APPLICATION VULNERABILITY'],
        ['ATTACK_SURFACE', 'Attack Surface : Number of any information that related to domain or ip address that can be used as attack vector',
            'OPERATING_SYSTEM,CO_HOSTED_SITE,UDP_PORT_OPEN,TCP_PORT_OPEN,IP_ADDRESS,WEBSERVER_HTTPHEADERS,DEVICE_TYPE,TCP_PORT_OPEN_BANNER,WEBSERVER_BANNER,WEBSERVER_TECHNOLOGY', 'ATTACK SURFACE'],
        ['SOCIAL_ENGGINEERING', 'Social Engineering :Number of any leak email address related to domains and ip address',
            'EMAILADDR_COMPROMISED', 'SOCIAL ENGINEERING'],
        ['HACKER_ACTIVITY', 'Hacker Activity :Number of hacker acvtivities related to domains or ip address',
            'DEFACED_INTERNET_NAME,DEFACED_IPADDR,DEFACED_AFFILIATE_INTERNET_NAME,DEFACED_COHOST,DEFACED_AFFILIATE_IPADDR,DARKNET_MENTION_URL,DARKNET_MENTION_CONTENT', 'HACKER ACTIVITY'],
    ]
        ...
        with self.dbhLock:
            ...
            if init:
                ...
                for row in self.categories:
                    category = row[0]
                    category_descr = row[1]
                    category_types = row[2]
                    category_title = row[3]
                    qry_category = "INSERT INTO tbl_categories (category, desc, types, title) VALUES (?, ?, ?, ?)"

                    try:
                        self.dbh.execute(qry_category, (
                            category, category_descr, category_types, category_title
                        ))
                        self.conn.commit()
                    except Exception:
                        continue
    ...
    def scanInstanceCreate(self, instanceId: str, scanName: str, scanTarget: str, createdBy: str) -> None:
        ...
        qry = "INSERT INTO tbl_scan_instance \
            (guid, name, seed_target, created, status, created_by) \
            VALUES (?, ?, ?, ?, ?, ?)"

        with self.dbhLock:
            try:
                self.dbh.execute(qry, (
                    instanceId, scanName, scanTarget, time.time() * 1000, 'CREATED', createdBy
                ))
                self.conn.commit()
            except sqlite3.Error as e:
                raise IOError(
    ...
    def scanEventStore(self, instanceId: str, sfEvent, truncateSize: int = 0) -> None:
        ...
        Raises:
            ...
            IOError: database I/O failed
        """
        from cts import SpiderFootEvent
    ...
    def scanProfileInstanceCreate(self, name: str, slug: str, description: str, modules: str) -> None:

        qvars = [name, slug, description, modules]
        qry = """
            INSERT INTO tbl_custom_profiles (name, slug, description, modules) VALUES (?, ?, ?, ?);
        """
        ...
        try:
            ...
            self.log.warning("Unable to create instance in DB: " + e.args[0])

        return True
    # Get all scan profiles

    def scanProfileInstanceList(self):
        qry = """
            SELECT * FROM tbl_custom_profiles;
            """
    ...
    # Get scan profile
    def scanProfileInstanceGet(self, slug: str) -> list:
        qry = """
            SELECT * FROM tbl_custom_profiles WHERE slug = ?;
            """
    ...
    def scanProfileInstanceUpdate(self, scanprofile_id, name, slug, description, modules):
        qry = """
            ...
            SET name = ?, slug = ?, description = ?, modules = ?
            WHERE id = ?
        """

        qvars = [name, slug, description, modules, scanprofile_id]
    ...
    def scanProfileInstanceDelete(self, slug):
        qry = """
            DELETE FROM tbl_custom_profiles WHERE slug = ?;
        """

        qvars = [slug]
        self.log.warning("masuk database delete")

        try:
            self.dbh.execute(qry, qvars)
            self.conn.commit()
        except sqlite3.Error as e:
            self.sf.fatal("Unable to delete instance in DB: " + e.args[0])

        return True

    # Create a new user
    def userInstanceCreate(self, name, password, salt, role):
        qry = """
            INSERT INTO tbl_users
            (name, password, salt, role) 
            VALUES (?, ?, ?, ?)
        """
        qvars = [name, password, salt, role]
        try:
            self.dbh.execute(qry, qvars)
            self.conn.commit()
        except sqlite3.Error as e:
    ...
    def userInstanceGet(self, name):
        ...
        try:
            ...
            self.sf.fatal("Unable to get instance in DB: " + e.args[0])

    # Update user password
    def userInstancePasswordUpdate(self, name, password, salt):
        qry = """
            UPDATE tbl_users 
            SET password = ?, salt = ?
            WHERE name = ?
        """
        qvars = [password, salt, name]
    ...
    # Update API key
    def userInstanceApiKeyUpdate(self, name, api_key):
        qry = """
            UPDATE tbl_users 
            SET api_key = ?
            WHERE name = ?
    ...
    # Delete user
    def userInstanceDelete(self, user_id):
        qry = """
    ...
    # Check user api key
    def userInstanceApiKey(self, api_key):
        qry = """
            SELECT name, api_key FROM tbl_users WHERE api_key = ?
        """
        qvars = [api_key]
        try:
            self.dbh.execute(qry, qvars)
    ...
    def userInstancesCount(self):
        ...
        try:
            ...
            self.sf.fatal("Unable to get instances count in DB: " + e.args[0])

    # Update last active date
    def userInstanceUpdateLastActive(self, name):
        qry = """
            UPDATE tbl_users 
